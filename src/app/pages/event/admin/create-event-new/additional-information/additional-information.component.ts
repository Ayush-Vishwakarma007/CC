...
export class AdditionalInformationComponent implements OnInit {
  ...
  @Output() addInfoChange: EventEmitter<any> = new EventEmitter();
  ...
  getformStepsWithFields() {
    let data = {
      path: "community/formStep/getAll/"+this.eventId,
      isAuth: true
    };
  ...
  getFieldType() {
    let data = {
      path: "auth/membershipFieldsType",//panding
      isAuth: true
    };
    this.apiService.get(data).subscribe(response => {
      if (response['status']['status'] == 'OK') {
  ...
  getFieldDetailById(id, index) {
    let data = {
      path: "community/field/details/" + id,
      isAuth: true
    };
  ...
  submitSection(id = '', index = '', fieldId = '') {
    if (this.sectionForm.valid) {
      ...
      if (id != '') {
        ...
        data = {
          path: "community/formStep/" + id,
          data: formData,
          isAuth: true
        };
      } else {
        formData['index'] = this.formStepsWithFields.length+1;
        formData["referenceId"] = this.eventId;
        data = {
          path: "community/formStep",
          data: formData,
          isAuth: true
        };
      }
  ...
  removeSection(id)
  {
    ...
    Swal.fire({
      ...
      if (result.value) {
        let data = {
          path: "community/formStep/" + id,//deleteFormStep
          isAuth: true
        };
  ...
  submitField(index, id = '') {
    ...
    if(this.fieldForm.value.id != '' && this.fieldForm.value.id !=  null)
    {
      editTime = true;
      data = {
        path: "community/field/"+this.fieldForm.value.id,//createOrUpdateField
        data: formData,
        isAuth: true
      };
    }else {
  ...
  removeFiled(index,id)
  {
    ...
    Swal.fire({
      ...
      if (result.value) {
        let data = {
          path: "/community/field/" + id,//deleteField
          isAuth: true
        };
