...
@Component({
  selector: 'app-edit-my-chapter-sponsorship',
  templateUrl: './edit-my-chapter-sponsorship.component.html',
  styleUrls: ['./edit-my-chapter-sponsorship.component.scss']
})
export class EditMyChapterSponsorshipComponent implements OnInit {
  ...
  getSponsorshipDetail() {
    let request = {
      path: 'event/chapter/sponsor/details/' + this.sponsorId,
      isAuth: true
    }
  ...
  submit() {
    ...
    if (this.addSponsorForm.valid) {
      ...
      let data = {
        path: "event/chapter/sponsorship/edit/" + this.sponsorId,
        data: formData,
        isAuth: true
      };
      ...
      this.apiService.post(data).subscribe(response => {
        if (response['status']['code'] == 'CREATED' || response['status']['code'] == 'OK') {
          ...
          this.isSubmit = false;
          this.router.navigate(['/my-contribution/chapter-my-sponsorship'])
        } else {
